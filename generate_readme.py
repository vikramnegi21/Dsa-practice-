...
def compute_streak(problems):
    ...
    return max(streak, MANUAL_STREAK), max(longest, MANUAL_STREAK)
...
def generate_heatmap_svg(problems):
    ...
    max_count = max((counts.get(d, 0) for c in grid for d in c), default=1)
    ...
    month_svg = "".join(
        f'<text x="{PAD_L + ci*(CELL+GAP)}" y="{PAD_T-6}" '
        f'font-family="Segoe UI" font-size="9" fill="#484f58">{lbl}</text>\n'
        for ci, lbl in month_lbls.items()
    )
    day_svg = "".join(
        f'<text x="2" y="{PAD_T + ri*(CELL+GAP) + CELL - 2}" '
        f'font-family="Segoe UI" font-size="9" fill="#484f58">{lbl}</text>\n'
  ...
  <text x="{PAD_L}" y="{H-6}" font-family="Segoe UI" font-size="9" fill="#484f58">{total_logged} problems logged</text>
...
# ─── RECENT ACTIVITY (markdown table — clickable) ─────────
def generate_recent_activity_md(problems, days=7):
    ...
    if not recent:
        return "_No problems logged in last 7 days._\n"

    def diff_badge(d):
        dl = str(d).strip().lower()
        if dl == 'easy':
            return '![Easy](https://img.shields.io/badge/Easy-2ecc71?style=flat-square&logoColor=black)'
        if dl == 'medium':
            return '![Med](https://img.shields.io/badge/Medium-f39c12?style=flat-square&logoColor=black)'
        if dl == 'hard':
            return '![Hard](https://img.shields.io/badge/Hard-e74c3c?style=flat-square&logoColor=black)'
        # Codeforces numeric rating
        try:
            n = int(d)
            col = '2ecc71' if n <= 1000 else ('f39c12' if n <= 1500 else 'e74c3c')
    ...
    for i, (d, p) in enumerate(recent, 1):
        ...
        prob_cell = f"[{prob}]({link})" if link.startswith('http') else prob
        lines.append(f"| {i} | {prob_cell} | {platform} | {diff} | {date_str} |")
...
def build_readme(problems, lc):
    ...
    # ── RECENT ACTIVITY (markdown — fully clickable) ──
    a('## Recent Activity\n\n')
    a(recent_md)
    a('\n---\n\n')
...
def main():
    ...
    with open(README_FILE, "w", encoding="utf-8") as f:
        f.write(readme)
    print(f"Done — {len(problems_data)} problems, all files updated.")
